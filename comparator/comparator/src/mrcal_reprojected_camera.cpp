...
std::shared_ptr<MrCalReprojectedCamera>
MrCalReprojectedCamera::from_files(std::shared_ptr<CameraIntf> &&camera,
  ...
  return std::make_shared<MrCalReprojectedCamera>(
      std::move(camera), load_lens_model(rich_camera_model_file),
...
MrCalReprojectedCamera::MrCalReprojectedCamera(
    ...
    : _camera{camera}, _rich_lens_model{std::move(rich_lens_model)},
      _lean_lens_model{std::move(lean_lens_model)},
      _reprojection_maps{create_reprojection_map(
          *_rich_lens_model, *_lean_lens_model, _camera->get_resolution())} {
...
cv::Mat MrCalReprojectedCamera::get_frame() const {
  ...
  cv::remap(frame, remapped_frame, _reprojection_maps.map_x,
            _reprojection_maps.map_y, cv::INTER_LINEAR);
...
std::unique_ptr<mrcal_cameramodel_t>
load_lens_model(const std::string &file_path) {
...
cv::Mat
get_intrinsics_from_camera_model(const mrcal_cameramodel_t &camera_model) {
...
ReprojectionMaps
create_reprojection_map(const mrcal_cameramodel_t &rich_lens_model,
  ...
  const auto rays = [&points_to, &lean_lens_model] {
    ...
    mrcal_unproject(rays.data(), points_to.data(), points_to.size(),
                    &lean_lens_model.lensmodel, lean_lens_model.intrinsics);
    return rays;
  }();
  ...
  const auto projected_points = [&rays, &rich_lens_model] {
    ...
    mrcal_project(projected_points.data(),
                  NULL, // do not care about the gradients
                  NULL, // do not care about the gradients
                  rays.data(), rays.size(), &rich_lens_model.lensmodel,
  ...
  ReprojectionMaps maps{cv::Mat::zeros(resolution, CV_32FC1),
