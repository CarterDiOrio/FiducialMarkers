...
def load_data(filename) -> DistributionData:
    ...
    print(f"Mean XYZ: {np.mean(x)} {np.mean(y)} {np.mean(z)}")
    print(f"Std X: {std_x}")
    print(f"Std Y: {std_y}")
    print(f"Std Z: {std_z}")
    ...
    return DistributionData(
        ...
        mean_x, mean_y, mean_z, mean_roll, mean_pitch, mean_yaw, data)
...
def analyze_data(camera_filename, mount_filename):
    ...
    # rotation_errors = camera_dist.data[:,3:6] - np.mean(camera_dist.data[:,3:6], axis=0)
    # rotation_error_magnitude = np.linalg.norm(np.rad2deg(rotation_errors), axis=1)

    # plt.hist(rotation_error_magnitude, bins=100)
    # plt.xlabel('Rotation Error Magnitude (deg)')
    # plt.ylabel('Frequency')
    # plt.title('Histogram of rotation error magnitudes')
    # plt.show()
    ...
    # plt.plot(np.linspace(100, 2000, 25), mean_by_distance, label='Mean reproj error (px)')
    # plt.plot(np.linspace(100, 2000, 25), std_dev_by_distance, label='Std Dev reproj error (px)')
    # plt.plot(np.linspace(100, 2000, 25), mean_unprojection_error_by_distance, label='Mean unproj error (mm)')
    ...
    for N in range(1, 100_000, 1):
        ...
        for i in range(0, 5000):
            ...
            if reprojection_error_norm <= 1.0:
                successes.append(1)
            else:
                successes.append(0)
